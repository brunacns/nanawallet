...
- **Auditoria completa de segurança** (autenticação, RLS/isolamento entre usuárias, configuração do Supabase, GitHub, frontend/XSS, exportação/exclusão de dados, CSRF/sessão, CORS/rate limiting — relatório completo entregue à parte). Achados corrigidos nesta versão:
  - **XSS armazenado (severidade média)**: `emoji`/`cor` de categorias e carteiras eram inseridos em `innerHTML` sem escapar — uma chamada direta à API do Supabase (fora da interface, que só oferece emoji/cor fixos) poderia gravar HTML/JS malicioso nesses campos da própria conta. Corrigido em `categorias.js`, `carteiras.js`, `dashboard.js`, `ticketAlimentacao.js` (todo `emoji` agora passa por `escaparHtml`) e um novo `corSegura()` em `formatadores.js` (valida que `cor` é um hex de 6 dígitos antes de entrar num atributo `style=""`, caindo num cinza neutro se não for). Verificado com um payload de teste real (`<img src=x onerror=...>` e uma tentativa de quebra de atributo) confirmando que nenhum dos dois executa mais.
  - **RLS reforçada em profundidade**: as políticas de INSERT/UPDATE de `gastos`/`carteira_movimentacoes` passaram a exigir também que `categoria_id`/`carteira_id` referenciados pertençam à própria usuária (antes só validavam `user_id` da própria linha) — fecha uma referência cruzada teoricamente possível, mesmo que hoje inatingível na prática (o RLS de SELECT já impede descobrir o UUID de outra conta). Testado com ataques simulados via SQL (dentro de transação com rollback, nunca tocando dado real): inserir gasto com `categoria_id` alheio e com `user_id` falsificado — os dois bloqueados corretamente pela RLS.
  - **CSP também na versão Web**: o app Desktop já tinha uma Content-Security-Policy restritiva aplicada pelo Tauri, mas a versão Web (GitHub Pages, hospedagem estática sem cabeçalhos customizáveis) não tinha nenhuma — adicionada via `<meta http-equiv="Content-Security-Policy">` no `index.html`, equivalente à do Desktop.
  - **Confirmado, com um teste real e não-destrutivo (conta de teste criada e apagada logo em seguida)**: o cadastro público no Supabase **ainda está aberto** — mesmo com o botão removido da interface (etapa anterior), uma chamada direta à API cria uma conta nova instantaneamente. RLS impede essa conta de ver dados da usuária real, mas viola o pedido explícito de acesso restrito a uma única pessoa. **Não corrigido automaticamente** — é uma configuração de Auth do painel do Supabase (Authentication → Sign In / Providers → Email → "Allow new users to sign up"), fora do alcance das ferramentas disponíveis aqui; documentado como ação manual obrigatória no relatório.
- Suíte de testes (`npm test`, 99 testes) e o app inteiro (login, CRUD de gasto/ganho/lembrete/meta, histórico, dashboard, exportação) testados de novo depois de todas as correções, sem regressão.
